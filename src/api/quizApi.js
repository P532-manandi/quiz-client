...
export const fetchQuestions = async () => {
  const response = await fetch(
    "https://quiz-service-bb46.onrender.com/questions"
  );
  ...
  if (Array.isArray(data.questions)) {
    return data.questions.map((q) => ({
      ...
      imageUrl:
        q.imageUrl ||
        `https://quiz-service-bb46.onrender.com/questions/${q.id}/image`,
    }));
  }
...
export const fetchQuizById = async (id) => {
  const response = await fetch(
    `https://quiz-service-bb46.onrender.com/quizzes/${id}`
  );
  return response.json();
};
...
export const saveQuiz = async (quiz, existingQuizId = null) => {
  ...
  try {
    ...
    const data = await response.json();
    alert(
      isUpdate ? "Quiz updated successfully!" : "Quiz created successfully!"
    );
    return true;
  } catch (error) {
